...
public class MarcoPrincipal extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        herramientasImagen.add(botonNuevo);
        ...
        herramientasImagen.add(botonAbrir);
        ...
        herramientasImagen.add(botonGuardar);
        herramientasImagen.add(jSeparator1);
        ...
        herramientasImagen.add(modoPunto);
        ...
        herramientasImagen.add(modoLinea);
        ...
        herramientasImagen.add(modoRectangulo);
        ...
        herramientasImagen.add(modoElipse);
        herramientasImagen.add(jSeparator2);
        ...
        herramientasImagen.add(seleccionColores);
        ...
        herramientasImagen.add(selectorGrosor);
        ...
        herramientasImagen.add(modoEditar);
        ...
        herramientasImagen.add(modoRelleno);
        ...
        herramientasImagen.add(modoTransparencia);
        ...
        herramientasImagen.add(modoAlisar);
        ...
        barraSuperior.add(herramientasImagen);
        ...
        barraSuperior.add(herramientaSonido);
        ...
        barraSuperior.add(herramientasVideo);
        ...
        menuArchivo.add(abrirAudio);

        abrirVideo.setIcon(new javax.swing.ImageIcon(getClass().getResource("/paintbasico2d/iconos/AbrirVideo.png"))); // NOI18N
    ...
    */
    ...
    /**
     * La clase bandCombineOp nos permite cambiar el color de las bandas de una
     * imagen en funcion de las demas y de una matriz de transformacion. La 
     * operacion consistiria, por tanto, en multiplicar cada pixel (que vendria 
     * representado como un vector con 3 valores correspondientes a las bandas 
     * de una imagen) por una matriz de transformacion (3x3). La multiplicacion 
     * seguiria el orden inverso al aqui mencionado, ya que queremos que cada
     * valor RGB del nuevo pixel corresponda a la suma de la multiplicacion de 
     * cada elemento de una misma fila de la matriz por el elemento 
     * correspondiente del vector RGB del pixel.
     * 
     * En el ejemplo representado abajo, nuestra matriz de transformacion 
     * intercambiara en cada pixel el valor de la banda verde con el de la banda
     * azul, manteniendo la banda roja con el mismo valor.
     * 
     * En este codigo tambien hay que señalar la aparicion de los objetos Raster,
     * los cuales representan la matriz rectangular de pixeles (podriamos decir,
     * por tanto, que mantiene la estructura de una imagen, tanto datos como 
     * posicion de cada pixel).
     * 
     * Ademas, un objeto raster contiene 2 atributos remarcables como son 
     * dataBuffer y sampleModel. El primero (de tipo DataBuffer) se encargara de
     * almacenar los datos de la imagen (los valores de cada pixel). De esta 
     * clase heredan diferentes subclases, dependiendo de que tipo de datos este
     * almacenando dicho objeto
     * 
     * El segundo de los atributos mencionados (de tipo SampleModel) 
     * es el encargado de organizar el vector de datos. De esta clase tambien 
     * heredan otras 3, las cuales son:
     * 
     * - ComponentSampleModel, la cual organiza la informacion por componentes,
     * es decir, en cada posicion del vector de datos se almacenara la 
     * informacion correspondiente a un componente de un determinado pixel. A su
     * vez de aqui heredan dos clases mas:
     * 
     *      - BandedSampleModel: Para cada banda de la imagen existira un buffer
     *      de datos distinto.
     * 
     *      - PixelInterleavedSampleModel: La informacion de cada pixel 
     *      se almacena en el mismo buffer de datos.
     * 
     * - SinglePixelPackedSampleModel: En una posicion del vector de datos 
     * tendremos toda la informacion sobre las bandas de un pixel. Esto se 
     * consigue codificando los bits de un elemento del vector de datos de 
     * manera que, al descodificarlo, obtengamos como resultado la informacion
     * necesaria para la representacion de un pixel. 
     * 
     * - MultiPixelPackedSampleModel: Similar al anterior con la diferencia de
     * que en una misma posicion del buffer de datos es capaz de almacenar mas
     * de un pixel.
     * 
     * @param evt 
     */
    ...
    /**
     * La clase ColorSpace define el espacio de color de una imagen. Con la 
     * clase ColorConvertOp, por tanto, podremos aplicar un espacio de color u 
     * otro a una imagen. 
     * 
     * Para este ejemplo como podemos observar aplicamos el espacio de color 
     * gris que nos permite ver una imagen con distintas tonalidades de blanco
     * y negro. 
     * 
     * El espacio de color, dentro de una imagen de tipo BufferedImage, se 
     * asocia con un ColorModel. Este objeto define como se codificara el color
     * de una imagen, ademas de contener metodos los cuales nos proporcionaran
     * la capacidad de traducir los valores de un pixel (almacenados en el vector
     * de datos) a sus distintos componentes, para poder visualizar la imagen.
     * 
     * Dependiendo de que modelo de muestreo estemos usando, se usara uno de los
     * siguientes tipos de objeto que heredan de la clase ColorModel:
     * 
     * - ComponentColorModel: Se debe usar un modelo de muestreo basado en 
     * componentes, ya que con este ColorModel interpretamos que cada posicion 
     * del vector de datos representa un componente distinto de un pixel.
     * 
     * - IndexColorModel: En este caso el DataBuffer debe contener un valor que 
     * servira como indice a una tabla donde se indicara que valor tiene un 
     * determinado pixel.
     * 
     * - PackedColorModel: En este caso el modelo de muestreo deberia ser 
     * empaquetado, es decir, cada elemento del vector de datos es un valor 
     * numerico codificado que representara los componentes de un pixel.
     * 
     * @param evt 
     */
        ...
        if(ventanaActiva!=null){
            ...
            if(img!=null){
                try{
                    ...
                    ventanaActiva.getLienzo().setImagenFondo(imgdest);
                    ventanaActiva.getLienzo().repaint();
                }
                catch(IllegalArgumentException e){
                    System.err.println(e.getLocalizedMessage());
    ...
    /**
     * Accion para abrir un audio y añadirlo a la lista de reproduccion. Cuando
     * hayamos seleccionado el archivo, para que aparezca su nombre en el 
     * comboBox correspondiente, tenemos que sobreescribir el metodo toString, 
     * parar que en la lista desplegable salga el nombre del archivo correctamente.
     * 
     * Se usan filtros tanto para tipo wav como para tipo au, que seran formatos
     * de audio reproducibles para nuestro programa.
     * @param evt 
     */
    
    private void abrirAudioActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_abrirAudioActionPerformed
        JFileChooser dlg=new JFileChooser();
        FileFilter filtrowav=new FileNameExtensionFilter("Archivos .wav", "wav");
        FileFilter filtroau=new FileNameExtensionFilter("Archivos .au", "au");
        ...
        if(resp==JFileChooser.APPROVE_OPTION){
            try{
                ...
                System.err.println("Error al leer el archivo de sonido");
                
                MiDialogo diag=new MiDialogo(this,true,"Error al leer el archivo de sonido");
                diag.setLocation((this.getWidth()/2)-diag.getWidth()/2,
                                 (this.getHeight()/2)-diag.getHeight()/2);
                diag.setVisible(true);
            }
    ...
    /**
     * Se crea una nueva VentanaInternaVideo la cual tendra asociada el archivo
     * seleccionado a traves del dialogo desplegado. Esta ventana se asociara con
     * el atributo escritorio y se hara visible.
     * @param evt 
     */
